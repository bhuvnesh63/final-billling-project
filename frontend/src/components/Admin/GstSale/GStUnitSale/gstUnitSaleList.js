...
const SaleUrl = "http://localhost:4000/api/v1/gstunitorders"


const GstBagSaleList = () => {
    ...
    const deleteData = (id) => {
        ...
        axios.delete(`http://localhost:4000/api/v1/bagsaleorder/${id}`).then(response => {
            ...
            toast.success("Unit Order deleted Succesfully")
        })
            .catch(error => {
                console.log(error)
            })
    ...
    const filterSaleOrders = (saleOrders) => {
        if (searchInput.trim() !== '') {
    ...
    useEffect(() => {
        filterSaleOrders(getsale?.GStUnitorders || []);
    }, [searchInput, getsale]);
                                    ...
                                    {filteredSaleOrders.length === 0 ? (
                                        ...
                                        getsale.GStUnitorders.map((saleOrder, index) => (
                                            <tr key={saleOrder._id}>
                                                ...
                                                <td>
                                                    <Link to={`/editgstunitsale/${saleOrder._id}`}>
