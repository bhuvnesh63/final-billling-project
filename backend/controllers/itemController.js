...
exports.createItem = (async (req, res, next) => {
    ...
    //     req.body.image = req.file.filename;
    //   }
    const item ={...req.body, createdDate: new Date() };
...
exports.getAllitemsbydate = async (req, res) => {

    const date1 = new Date();
    const date2 = date1.setMonth(date1.getMonth() -1)
    date1.setHours(0, 0, 0)
    const apiFeature = new ApiFeatures(Item.find(
        {
            createdDate: {
                $gte: new Date(date1),
                $lte: new Date()
    ...
    const itemsbyDate = await apiFeature.query;
...
exports.getItemDetailsByDate = async (req, res) => {
